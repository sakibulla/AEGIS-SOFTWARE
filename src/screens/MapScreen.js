  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {/* ── Map Placeholder (SVG replaced with div) ── */}
        <View style={styles.mapContainer}>
          <View style={styles.mapPlaceholder}>
            <Ionicons name="map" size={48} color={colors.textMuted} />
            <Text style={styles.mapPlaceholderText}>Live SLAM Map</Text>
            <Text style={styles.mapPlaceholderSub}>
              {selectedBot ? `${selectedBot} location` : 'Select a bot to view'}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.legendRow}>
          {BOTS.map(bot => (
            <TouchableOpacity
              key={bot.id}
              style={[styles.legendItem, selectedBot === bot.id && { borderColor: bot.color, backgroundColor: colors.cyanFaint }]}
              onPress={() => setSelectedBot(selectedBot === bot.id ? null : bot.id)}
            >
              <View style={[styles.legendDot, { backgroundColor: bot.color }]} />
              <Text style={[styles.legendLabel, { color: selectedBot === bot.id ? colors.cyan : bot.color }]}>{bot.name}</Text>
            </TouchableOpacity>
          ))}
        </View>
...
const getStyles = (colors) => StyleSheet.create({
  ...
  mapPlaceholderText: { fontSize: Typography.md, fontWeight: Typography.bold, color: colors.textPrimary, marginTop: Spacing.md },
  mapPlaceholderSub: { fontSize: Typography.sm, color: colors.textSecondary, marginTop: Spacing.sm },
